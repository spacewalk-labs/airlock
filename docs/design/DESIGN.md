...
description: >-
  ...
  DESIGN.dark.md — this file is the light override.
colors:
  canvas: "#f5f5f7"
  surface1: "#ffffff"
  surface2: "#eceff2"
...
**Dark is canonical.** `DESIGN.dark.md` holds the values that ship by default;
this file is the light override. Structure, typography, spacing and radii are
...
|---|---|---|
| `canvas` | `#f5f5f7` | The page. |
| `surface1` | `#ffffff` | Cards, panels, raised rows. |
| `surface2` | `#eceff2` | Nested surface, hover fill, and the app tile. |
| `hairline` | `#d2d2d7` | Borders. Never a text colour. |
...
| `body` | `#4a4a4f` | Secondary text and long-form reading. |
| `mute` | `#646469` | Labels, timestamps, disabled. |

**Primary** — the only strong colour on a screen.

| Token | Value | What it is for |
|---|---|---|
| `primary` | `#0066cc` | Links, focus, primary action, emphasis. |
| `onPrimary` | `#ffffff` | Text placed on top of the primary. |

**State** — reserved.

| Token | Value | What it is for |
|---|---|---|
| `warning` | `#9a6700` | Something needs attention. |
| `danger` | `#cf222e` | Something is broken or destructive. |
...
- **Resting** — `0 3px 8px -2px rgba(0,0,0,.20), 0 1px 2px rgba(0,0,0,.10)`
- **Hover** — `0 9px 18px -5px rgba(0,0,0,.28), 0 2px 5px rgba(0,0,0,.12)`,
...
The brand mark is a circle. It is the only true circle in the system.
