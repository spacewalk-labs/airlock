...
#   HTPASSWD_DIR HTPASSWD_BIN TOKEN_HEADER DOC_URL DOC_AUDIENCE TITLE_META
render_publish_unit_service() {
  ...
  local STATE_DIR="$1" GATED_DIR="$2" HTPASSWD_DIR="$3" HTPASSWD_BIN="$4" \
        TOKEN_HEADER="$5" DOC_URL="${6:-}" DOC_AUDIENCE="${7:-}" TITLE_META="${8:-false}"
...
UNIT
  if [ "$TITLE_META" = true ]; then
    echo 'Environment=AIRLOCK_PUBLISH_TITLE_META=true'
  fi
  cat <<UNIT
...
Environment=AIRLOCK_PUBLISH_DOC_URL=${DOC_URL}
Environment=AIRLOCK_PUBLISH_DOC_AUDIENCE=${DOC_AUDIENCE}
