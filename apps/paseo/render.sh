...
# Bounds every restart, clean-exit or failed (Restart=always below): three in
# thirty seconds, then the unit stays failed for a human to look at.
...
#
# One @getpaseo/server on this prefix, and it is the patched one. Node resolves
# upward from the cli, so a copy nested under it — what \`npm i -g @getpaseo/cli\`
# produces, and what any install from outside this installer leaves behind — wins
# over the prefix-level bundle that every patch in apps/paseo/install.sh edits.
# Measured on an operator box: a stray registry install on 2026-09-20 re-created the nested
# copy, the 2026-09-22 boot bound the daemon to it, and the web UI came back with
# upstream font sizes and sidebar order while the patched copy sat inert beside it.
# The installer already removes a shadow it meets (install/test-paseo-bundle-install.sh
# section 3), but nothing runs between installs, and the resolution is bound HERE, at
# start. So the invariant is enforced here rather than reported: the prefix can only
# offer the canonical copy.
#
# Guarded on the prefix-level copy existing, which is exactly the bundle layout this
# installer owns — registry mode has no prefix-level server and legitimately runs from
# the nested one, so it is left alone. Unlike the pidfile above, this touches only this
# installer's own fixed npm prefix (\$HOME/.npm-global, install.sh PASEO_PREFIX; derived
# from HOME here the same way PASEO_HOME is), never another daemon's live singleton
# state, and it is idempotent on every Restart= retry. It cannot fail the unit: a
# removal that does not happen leaves exactly today's behaviour, and a boot loop would
# be worse than an unpatched UI.
ExecStartPre=/bin/sh -c '[ -d ${HOME}/.npm-global/lib/node_modules/@getpaseo/server ] && rm -rf ${HOME}/.npm-global/lib/node_modules/@getpaseo/cli/node_modules/@getpaseo/server || true'
...
# (status 0), and a duplicate singleton or broken configuration exits non-zero
# and must remain failed for diagnosis. on-success served both and missed a
# third case: a transient failure. 2026-09-11 this daemon hit ENOSPC writing
# its rotating log, exited 1, and stayed down 5.5 hours on a box that had
# space again within minutes — nothing retries a non-zero exit, and nothing
# said so. ENOSPC and a broken config share an exit status, so the code cannot
# separate them; StartLimitBurst above can. Restart=always retries either at
# most three times in thirty seconds and then leaves the unit failed — the
# diagnosable end state, with the 53-attempt loop that motivated on-success
# bounded out by that same limit. Every other app unit in this repo restarts
# on failure; paseo was the outlier.
