...
// 배율 저장소와 변수 이름은 공용 문서 뷰어와 같다 — 같은 브라우저에서는
// 어떤 문서를 열어도 같은 크기로 읽힌다.
...
var theme=String(get(TK,"")||get(AK,"")||"auto").toLowerCase();if(T.indexOf(theme)<0)theme="auto";
function dark(){return theme==="dark"||(theme==="auto"&&matchMedia("(prefers-color-scheme: dark)").matches)}
function at(){var r=document.documentElement;if(theme==="auto")r.removeAttribute("data-theme");else r.setAttribute("data-theme",theme);var b=document.querySelector('[data-reader-action="theme"]');b.textContent=dark()?"☀":"☾";b.title="테마: "+LABEL[theme];b.setAttribute("aria-label","테마: "+LABEL[theme]);put(TK,theme);
// 앱 셸(목록 화면)은 아직 자기 키를 읽는다. 허브 전체의 테마 정본이 정해질 때까지의 이음매라,
// 문서에서 바꾼 테마가 목록에도 반영되도록 같이 적어 준다. TESTABLE:READER_THEME_APP_MIRROR
put(AK,LABEL[theme])}
function ct(){theme=T[(T.indexOf(theme)+1)%T.length];at();toast("테마 "+LABEL[theme])}
