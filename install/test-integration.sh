...
# DEVTERM_INDEPENDENCE phase 4: panel.html, accounts.js and the thin account control
# are gone from devterm. The secret-drop adapter remains an owner-gated alias because
# the terminal still needs its same-origin entry point.
for _loc in /panel.html /accounts.js; do
  if grep -qF "location = $_loc {" "$DTFRAG" 2>/dev/null; then
    bad "devterm still serves retired account alias $_loc"
  else
    ok "devterm retired account alias $_loc"
  fi
done
_secret_blk="$(awk '
  index($0, "location = /secretdrop.js {") { inside = 1 }
  inside { print }
  inside && /^    }$/ { exit }
' "$DTFRAG" 2>/dev/null)"
if [[ "$_secret_blk" == *"alias "*"/assets/accounts/secretdrop.js;"* ]] \
   && [[ "$_secret_blk" == *'if ($owner_ok = 0) { return 403; }'* ]]; then
  ok "devterm retains the owner-gated platform secret adapter"
else
  bad "devterm secret adapter alias or its owner guard is missing"
fi
if grep -q '\[dry\] install web/ ->.*+ ui.js/popup.css)' "$TMP/orch.log" 2>/dev/null \
   && ! grep -q 'platform-account-control.js' "$TMP/orch.log" 2>/dev/null; then
  ok "devterm installs terminal UI only, without an account consumer"
else
  bad "devterm web install still carries account-control material"
